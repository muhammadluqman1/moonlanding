...
const DynamicPage = ({ doc }: DynamicPageProps): ReactElement => {
  // console.log(doc);
  const cart = useCart();
  ...
  useEffect(() => {
    document.querySelector("body").classList.add(`page-`+doc.uid);
...
DynamicPage.getInitialProps = async (ctx) => {
  ...
  const doc = await Client(req).getByUID('page', query.uid, {});
  if (!doc) {
