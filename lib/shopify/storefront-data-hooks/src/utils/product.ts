...
export function prepareVariantsWithOptions(
  ...
  return variants.map((variant) => {
    ...
    const optionsDictionary = variant.selectedOptions?.reduce((options: any, option: any) => {
...
/*
  prepareProductTitle()
  This function prepares a product title.
  This is sepcific for Enso Rings.
*/
export const prepareProductTitle = (title: string) => {
  return title.split('-')[0].trim();
};

/*
  prepareProductColorName()
  This function prepares the product's color name.
  This is specific for Enso Rings.
*/
export const prepareProuctColorName = (title: string) => {
...
export function prepareVariantsImages(
  ...
  //   [optionKey]: image
  // }
  const imageDictionary = variants.reduce<Record<string, ShopifyBuy.Image>>((images, variant) => {
